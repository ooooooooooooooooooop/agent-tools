...
| Briefing | canonical 编译产物 `briefing.md`（七段、≤8KiB）以 `systemPrompt.section` 挂载——永远在场、每次 assemble 重读、不竞争 user 消息；STALE 检测 |
...
- `distiller.py` — A：closed episodes→evidence packet→problem object；抽象器协议（none/file/template）+ grounding validator→MODEL_PROPOSAL
- `hypothesis_search.py` — B：problem object→H4（8 算子库+applicability）；统一 worker 契约；`--mark` 写 HYPOTHESIS_STATE_UPDATE
- `u1_accept.py` — U1 governed apply：唯一 canonical 写者；proposal→canonical + rollback 备份 + provisional 状态
...
端到端（已在真实会话验证）：episodes→packet→problem→worker 抽象→validator→MODEL_PROPOSAL→H4→判别 probe→HYPOTHESIS_STATE_UPDATE→晋升→U1 accept→canonical→briefing 跨会话生效。
