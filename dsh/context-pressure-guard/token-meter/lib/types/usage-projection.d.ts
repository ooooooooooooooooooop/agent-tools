...
/**
 * The token-usage unit's state schema 鈥?the one definition of the state
 ...
 * consistency is claimed 鈥?the pair is explicitly not one atomic request
 ...
 * `projectedTokens` 鈥?the sample plus the surface's signed movement since it
 * was taken 鈥?so occupancy answers for the next request rather than the last
