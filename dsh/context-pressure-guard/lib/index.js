...
var ReactLoopAgent = class {
	...
	*/
	async buildRequest(turn, step, tools, system, boundaryMessages, signal) {
		...
		const loopService = this.loopCtx.get("agentLoop");
		const pendingAdmissionMaxTokens = loopService.pendingAdmissionMaxTokens.get(this) ?? loopService.pendingAdmissionMaxTokensBySession.get(this.session);
		const seedConfig = deepFreeze(structuredClone(this.requestHeaderLogged ? {
			...requestProposal(persistedHeader),
			...pendingAdmissionMaxTokens === void 0 ? {} : { maxTokens: pendingAdmissionMaxTokens }
		} : {
			...route,
		...
		// Resolve capacity before preparing the adapter call.  The prepared-call
		// fingerprint is intentionally strict, so changing maxTokens on the
		// resulting header after prepareCall would make every safe request fail
		// closed at dispatch.
		const admissionPolicy = resolveAdmissionPolicy(loopService.config, proposedConfig.provider, proposedConfig.model);
		validateAdmissionPolicy(admissionPolicy);
		let resolvedContextWindow;
		try {
			const resolvedModel = await this.loopCtx.llm.resolveModelInfo(proposedConfig.provider, proposedConfig.model, signal);
			resolvedContextWindow = resolvedModel.context?.contextWindow;
		} catch (error) {
			// Middleware may serve an unregistered route; the normal LLM path still
			// owns the terminal adapter lookup in that case.
			if (!(error instanceof LlmError) || error.code !== "NO_ADAPTER") throw error;
		}
		signal.throwIfAborted();
		let finalConfig = proposedConfig;
		if (resolvedContextWindow !== void 0) {
			const toolNames = /* @__PURE__ */ new Map();
			const piMessages = [];
			for (const message of boundaryMessages) {
				if (message.role === "assistant") {
					const content = [];
					for (const block of message.content) if (block.type === "text") content.push({ type: "text", text: block.text });
					else if (block.type === "reasoning") content.push({ type: "thinking", thinking: block.text });
					else if (block.type === "tool-call") {
						toolNames.set(block.id, block.name);
						let args;
						try { args = JSON.parse(block.arguments); } catch { args = block.arguments; }
						content.push({ type: "toolCall", id: block.id, name: block.name, arguments: args });
					}
					piMessages.push({ role: "assistant", content, timestamp: 0, stopReason: "stop", usage: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, totalTokens: 0 } });
					continue;
				}
				const text = message.content.filter((block) => block.type === "text" || block.type === "reasoning").map((block) => block.text).join("");
				const results = message.content.filter((block) => block.type === "tool-result");
				if (text.length > 0 || results.length === 0) piMessages.push({ role: "user", content: text, timestamp: 0 });
				for (const result of results) piMessages.push({
					role: "toolResult",
					toolCallId: result.toolCallId,
					toolName: toolNames.get(result.toolCallId) ?? "unknown",
					content: [{ type: "text", text: result.content.filter((block) => block.type === "text" || block.type === "reasoning").map((block) => block.text).join("") || "(no output)" }],
					isError: result.isError ?? false,
					timestamp: 0
				});
			}
			const candidateHeader = canonicalHeader({
			...
			const preview = {
				messages: piMessages,
				...candidateHeader.system !== void 0 ? { systemPrompt: candidateHeader.system } : {},
				...candidateHeader.tools !== void 0 ? { tools: candidateHeader.tools.map((tool) => ({ name: tool.name, description: tool.description, parameters: tool.parameters })) } : {}
			};
			const estimatedInput = estimateAdmissionContext(preview);
			const requestedMaxTokens = proposedConfig.maxTokens;
			const decision = admissionDecision({
				configuredLimit: resolvedContextWindow,
				providerAttestedLimit: admissionPolicy.providerAttestedLimit,
				estimatedInput,
				inputMultiplier: admissionPolicy.inputMultiplier,
				safetyMargin: admissionPolicy.safetyMargin,
			...
			if (!decision.safe) {
				...
				loopService.blockedAdmissionCaps.set(this, decision);
				throw contextPreflightError(decision);
			}
			loopService.pendingAdmissionMaxTokens.delete(this);
			loopService.pendingAdmissionMaxTokensBySession.delete(this.session);
			loopService.blockedAdmissionCaps.delete(this);
			// Always materialize the effective output cap before prepareCall, even
			// when the caller omitted maxTokens. This keeps adapter defaults from
			// widening a request after admission.
			finalConfig = deepFreeze(structuredClone({ ...proposedConfig, maxTokens: decision.allowedOutput }));
		}
		let config;
		let preparedCall;
		try {
			preparedCall = await this.loopCtx.llm.prepareCall(finalConfig, signal);
		...
		const contextWindow = preparedCall?.context?.contextWindow ?? resolvedContextWindow;
...
const DEFAULT_CONTEXT_SAFETY_MARGIN = 16384;
const DEFAULT_OPERATIONAL_MAX_OUTPUT = 65536;
const DEFAULT_CONSERVATIVE_INPUT_MULTIPLIER = 1.08;
const CONTEXT_PREFLIGHT_BLOCKED_CODE = "CONTEXT_PREFLIGHT_BLOCKED";
/** Conservative route-local admission policy. Provider-attested limits are optional and never widen configured capacity. */
function resolveAdmissionPolicy(config, provider, model) {
	const route = config.contextAdmission?.routes?.find((item) => item.provider === provider && item.model === model);
	return {
		safetyMargin: route?.safetyMargin ?? config.contextAdmission?.safetyMargin ?? DEFAULT_CONTEXT_SAFETY_MARGIN,
		operationalMaxOutput: route?.operationalMaxOutput ?? config.contextAdmission?.operationalMaxOutput ?? DEFAULT_OPERATIONAL_MAX_OUTPUT,
		inputMultiplier: route?.inputMultiplier ?? config.contextAdmission?.inputMultiplier ?? DEFAULT_CONSERVATIVE_INPUT_MULTIPLIER,
		providerAttestedLimit: route?.providerAttestedLimit
	};
}
function validateAdmissionPolicy(policy, name = "contextAdmission") {
	if (!Number.isSafeInteger(policy.safetyMargin) || policy.safetyMargin < DEFAULT_CONTEXT_SAFETY_MARGIN) throw new TypeError(`${name}.safetyMargin must be a safe integer >= ${DEFAULT_CONTEXT_SAFETY_MARGIN}`);
	if (!Number.isSafeInteger(policy.operationalMaxOutput) || policy.operationalMaxOutput <= 0) throw new TypeError(`${name}.operationalMaxOutput must be a positive safe integer`);
	if (typeof policy.inputMultiplier !== "number" || !Number.isFinite(policy.inputMultiplier) || policy.inputMultiplier <= 1) throw new TypeError(`${name}.inputMultiplier must be a finite number > 1`);
	if (policy.providerAttestedLimit !== void 0 && (!Number.isSafeInteger(policy.providerAttestedLimit) || policy.providerAttestedLimit <= 0)) throw new TypeError(`${name}.providerAttestedLimit must be a positive safe integer`);
}
function effectiveContextLimit(configuredLimit, providerAttestedLimit) {
	if (!Number.isSafeInteger(configuredLimit) || configuredLimit <= 0) throw new TypeError("configured context window must be a positive safe integer");
	return providerAttestedLimit === void 0 ? configuredLimit : Math.min(configuredLimit, providerAttestedLimit);
}
function estimateAdmissionContext(context) {
	const textTokens = (text) => Math.ceil((text ?? "").length / 4);
	let tokens = textTokens(context.systemPrompt);
	if (context.tools?.length) tokens += textTokens(JSON.stringify(context.tools));
	for (const message of context.messages) {
		if (message.role === "user" || message.role === "toolResult") tokens += typeof message.content === "string" ? textTokens(message.content) : textTokens(JSON.stringify(message.content));
		else for (const block of message.content) if (block.type === "text") tokens += textTokens(block.text);
		else if (block.type === "thinking") tokens += textTokens(block.thinking);
		else tokens += textTokens(block.name) + textTokens(JSON.stringify(block.arguments));
	}
	return tokens;
}
function conservativeInputUpperBound(estimatedInput, multiplier) {
	if (!Number.isFinite(estimatedInput) || estimatedInput < 0) throw new TypeError("estimated input must be a non-negative finite number");
	return Math.ceil(estimatedInput * multiplier);
}
function admissionDecision({ configuredLimit, providerAttestedLimit, estimatedInput, inputMultiplier, safetyMargin, operationalMaxOutput, requestedMaxTokens }) {
	const limit = effectiveContextLimit(configuredLimit, providerAttestedLimit);
	...
	return deepFreeze({
		...
		conservativeInputUpperBound: conservativeInput,
		safetyMargin,
		operationalMaxOutput,
		allowedOutput,
		safe: allowedOutput > 0
	});
}
function contextPreflightError(decision) {
	return new LlmError(`local context preflight blocked: conservative input ${decision.conservativeInputUpperBound} + safety ${decision.safetyMargin} leaves no output within effective limit ${decision.effectiveLimit}`, CONTEXT_PREFLIGHT_BLOCKED_CODE);
}
//#endregion
...
var AgentLoop = class extends Service {
	...
	static Config = z.object({
		...
		contextAdmission: z.object({
			safetyMargin: z.number().step(1).min(DEFAULT_CONTEXT_SAFETY_MARGIN).default(DEFAULT_CONTEXT_SAFETY_MARGIN),
			operationalMaxOutput: z.number().step(1).min(1).default(DEFAULT_OPERATIONAL_MAX_OUTPUT),
			inputMultiplier: z.number().min(1.000001).default(DEFAULT_CONSERVATIVE_INPUT_MULTIPLIER),
			routes: z.array(z.object({
				provider: z.string().required(),
				model: z.string().required(),
				providerAttestedLimit: z.number().step(1).min(1),
				safetyMargin: z.number().step(1).min(DEFAULT_CONTEXT_SAFETY_MARGIN),
				operationalMaxOutput: z.number().step(1).min(1),
				inputMultiplier: z.number().min(1.000001)
			})).default([])
		}).default({}),
	...
	config;
	ownership;
	/** Per-agent one-shot safe completion cap recorded after a locally blocked oversized reservation. */
	pendingAdmissionMaxTokens = /* @__PURE__ */ new WeakMap();
	/** Session-keyed companion keeps the cap stable across wrapper identity boundaries. */
	pendingAdmissionMaxTokensBySession = /* @__PURE__ */ new WeakMap();
	/** Latest terminal local admission block, retained for diagnostics/recovery until a safe request succeeds. */
	blockedAdmissionCaps = /* @__PURE__ */ new WeakMap();
	...
	constructor(ctx, config) {
		...
		const contextAdmission = {
			safetyMargin: config.contextAdmission?.safetyMargin ?? DEFAULT_CONTEXT_SAFETY_MARGIN,
			operationalMaxOutput: config.contextAdmission?.operationalMaxOutput ?? DEFAULT_OPERATIONAL_MAX_OUTPUT,
			inputMultiplier: config.contextAdmission?.inputMultiplier ?? DEFAULT_CONSERVATIVE_INPUT_MULTIPLIER,
			routes: config.contextAdmission?.routes ?? []
		};
		validateAdmissionPolicy(contextAdmission);
		for (const [index, route] of contextAdmission.routes.entries()) validateAdmissionPolicy({
			safetyMargin: route.safetyMargin ?? contextAdmission.safetyMargin,
			operationalMaxOutput: route.operationalMaxOutput ?? contextAdmission.operationalMaxOutput,
			inputMultiplier: route.inputMultiplier ?? contextAdmission.inputMultiplier,
			providerAttestedLimit: route.providerAttestedLimit
		}, `contextAdmission.routes[${index}]`);
		this.config = {
			...config,
			contextAdmission,
...
//#endregion
export { AGENT_LOOP_SETTINGS_NAMESPACE, AGENT_LOOP_SETTINGS_SCHEMA, AgentLoop, AgentLoop as default, CONFIGURED_AGENT_IDENTITIES_KEY, CONTEXT_PREFLIGHT_BLOCKED_CODE, DEFAULT_CONSERVATIVE_INPUT_MULTIPLIER, DEFAULT_CONTEXT_SAFETY_MARGIN, DEFAULT_MAX_PARALLEL_TOOL_CALLS, DEFAULT_OPERATIONAL_MAX_OUTPUT, admissionDecision, conservativeInputUpperBound, effectiveContextLimit };
