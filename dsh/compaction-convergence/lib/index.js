...
/**
 * FIX B: 收敛失败识别。0.1.1-rc.2 无结构化错误类型；根据 summarizeCompaction
 * 抛出的固定前缀字符串识别。仅命中该前缀，不覆盖 transient/provider 失败。
 * 兼容债务：若上游引入结构化类型，应替换为 instance-of 判定。
 */
function isSummaryNotSmallerError(error) {
	return error instanceof Error && error.message.startsWith("summary is not smaller than the shadowed content");
}
/**
...
function selectCompactableRange(session, measurement, retainTokens) {
	...
	if (keepFromIdx === 0) return null;
	// FIX A: 从候选头部跳过连续 compaction checkpoint 节点；checkpoint 不得再次成为
	// shadowed source。候选区整体都是 checkpoint 时返回 null（不产生误导性压缩）。
	let startIdx = 0;
	while (startIdx < keepFromIdx) {
		const event = session.events[surfaceNodes[startIdx]];
		const source = event?.data?.source;
		if (source === void 0 || !isCompactCheckpointSource(source)) break;
		startIdx += 1;
	}
	// 跳过 checkpoint 后仍保持 tool-pairing balanced 的起点（checkpoint 自身两侧总是
	// balanced，所以通常紧随其后即可；向前多跳只是防御，不改变原语义）。
	while (startIdx < keepFromIdx) {
		if (toolPairingBalancedBefore(session, surfaceNodes[startIdx])) break;
		startIdx += 1;
	}
...
var BasicCompactionEngine = class extends CompactionEngine {
	...
	*/
	async compactIfNeeded(agent, trigger, signal) {
		...
		if (measurement.totalTokens < spec.thresholdTokens) return null;
		let result = null;
		const failedRegion = this.failedPressureRegion.get(agent);
		for (let attempt = 0; attempt <= spec.compactionRetries; attempt += 1) {
			...
			try {
				...
				if (isSummaryNotSmallerError(error)) {
					// 兼容债务：0.1.1-rc.2 无结构化错误类型，靠消息前缀识别，范围限定在
					// "summary is not smaller" 前缀；transient/provider 失败不会熔断。
...
//#endregion
export { BasicCompactionEngine, BasicCompactionEngine as default, isSummaryNotSmallerError, selectCompactableRange };
