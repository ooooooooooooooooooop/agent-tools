...
- `registry/gateways.yaml` 按 V2.1 SSOT 矩阵终态属于 personal-ai-state。**已于 Phase 3+4 Integration（2026-08-28）完成迁移**：真实 mapping 的唯一 canonical = personal-ai-private 私有仓库 `registry/gateways.yaml`；本仓库同名文件降级为 template/example（非 SSOT）。运行时查找顺序见 `scripts/trace_identity.py`（private canonical 优先）。
