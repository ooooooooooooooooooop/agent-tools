...
def canonical_conversation_id_from_url(url: str) -> str:
    ...
    Returns ``""`` for non-conversation URLs AND for ChatGPT's provisional
    ``/c/<NS>:<uuid>`` routes (observed: ``/c/WEB:<uuid>``) shown while a
    new conversation is being created. The provisional id is client-side —
    it is replaced by a different server-assigned id once the conversation
    persists — so it must never reach ``/backend-api/conversation/{id}``
    (returns HTTP 400) nor be stored as ``_current_conv_id``. Callers poll
    until this returns a real segment.
    """
    if not url or "/c/" not in url:
        return ""
    seg = url.split("/c/")[1].split("/")[0].split("?")[0]
...
class BackendClient:
    ...
    async def _conversation_id_from_url(self) -> str:
        ...
        try:
            ...
            return ""
        return canonical_conversation_id_from_url(url)
