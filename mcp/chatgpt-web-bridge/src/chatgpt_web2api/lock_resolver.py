...
def resolve_mutation_lock(
    ...
    # has_lockable_target (cdp_driver) covers owned tabs AND conv-bound shared
    # tabs — the lock key is the targetId itself, so two drivers attached to
    # the SAME conversation tab still serialize on the same lock file.
    # getattr fallback keeps the strict owned-only contract for drivers that
    # predate conv-affinity (tests, legacy construction).
    lockable = getattr(driver, "has_lockable_target", driver.has_owned_target)
    if lockable and target_id is not None:
