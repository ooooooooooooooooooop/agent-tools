        ...
        while time.monotonic() < deadline:
            try:
                result = await d._js_strict(
                    ...
                    # Locale-free first: subtract the thinking row's own
                    # innerText (.result-thinking is a class hook, not a
                    # localized label), then strip residual leading status
                    # labels for common locales — the English-only regex let
                    # "正在思考" through as answer text on zh UIs.
                    "  var thinkEl = last.querySelector('.result-thinking');"
                    "  var thinkText = thinkEl ? (thinkEl.innerText || '') : '';"
                    "  var bodyText = thinkText ? rawText.replace(thinkText, '').trim() : rawText;"
                    "  var text = mdText || bodyText"
                    "      .replace(/^Think(ing|\\s+for)[^\\n]*\\n?/i, '')"
                    "      .replace(/^(正在思考|思考中|正在推理|考えています|考え中|생각 중)[^\\n]*\\n?/, '');"
                    ...
                    # is_generating: the composer's stop button is visible.
                    # Locale-free (data-testid) and the hard "turn in progress"
                    # signal — used as a completion VETO below, covering the
                    # observed failure where a nearby action-button false
                    # positive fired ~1s after send on a fresh chat.
                    "  var stopBtn = document.querySelector('[data-testid=\"stop-button\"], button[aria-label*=\"Stop\" i], button[aria-label*=\"停止\"]');"
                    "  var is_generating = !!(stopBtn && (stopBtn.offsetParent !== null || stopBtn.getClientRects().length > 0));"
                    "  var hasThinkingEl = !!thinkEl;"
                    ...
                    # thinkingish: ungated by has_action (that gate suppressed
                    # is_thinking exactly when the false-positive completed the
                    # turn). True while only placeholder/status text exists.
                    "  var thinkingish = hasThinkingEl || (is_generating && !mdText && !bodyText);"
                    "  return JSON.stringify({text: text, md_text: mdText, html_len: html_len, child_count: child_count, has_action: has_action, is_thinking: is_thinking, is_generating: is_generating, thinkingish: thinkingish});"
                    "})()",
                )
            ...
            thinkingish = data.get("thinkingish", False)
            ...
            generation_active_signal = bool(is_thinking or is_generating)
