...
- User model authorization policy: Codex child agents use `{CODEX_CHILD_MODEL}` by default — both managed `explorer` and `worker`. Any other model or provider (including Terra, Sol, Claude/Fable/Opus, Gemini/Flash, and provider aliases) requires explicit authorization from the user in the current conversation. If Luna is unavailable, stop and report it; never choose another model as an automatic fallback. This policy does not rewrite the user's selected main-session model.
- This authorization rule takes precedence over every fallback or consultation rule below: no non-Luna fallback, second opinion, or external route may be started without fresh explicit user authorization.
- Native-first routing order is mandatory: for same-vendor labour, use native subagents first. A Codex brain uses the managed `explorer` (`{CODEX_CHILD_MODEL}`/low) and `worker` (`{CODEX_CHILD_MODEL}`/medium); a Claude brain uses managed `Explore` (`{claude_roles.get('reader') or 'haiku'}`) and `economy-worker` (`{claude_roles.get('workhorse') or 'sonnet'}`/medium). Do not use Agent Switchboard to launch same-vendor labour unless the named native role is unavailable or fails to start, and record that fallback. The external Antigravity Flash lane below is a distinct allowed Switchboard use, not a native child agent.
- Opposite-vendor consultation is opt-in, not automatic. A Codex brain should request a second opinion from Claude `{claude_chain}` only after explicit user authorization for that exact model; a Claude brain may use the live Codex frontier `{frontier}` only after the same authorization. If an authorized model is unavailable, stop and report it rather than silently selecting the next model.
- Capability tier outranks model version. Gemini Flash High is a useful, non-authoritative workhorse-level adviser; a higher version does not promote it above Sol/Fable or make its advice automatically authoritative. When Claude's `{claude_chain}` frontier chain is unavailable because of quota, reachability, entitlement, or another availability failure, a Codex brain may request a second opinion from the newest live Antigravity Flash High only after explicit user authorization, label it degraded advisory fallback, and retain final judgment.
- Cross-vendor routing must enter through Agent Switchboard's MCP tools whenever Switchboard is registered, and only after explicit user authorization for the requested non-Luna model. For Flash labour, the sender brain MUST call MCP `route_agent_task`; a request to use Flash "through CLI" means `surface="cli"` on that MCP call. The brain MUST NOT invoke `agy` in a shell or call `consult_antigravity` directly. Only the Switchboard backend may start `agy`; sender-side direct `agy` is prohibited.
- Codex, Claude, and Gemini brains should only proactively consider the newest live Antigravity Gemini Flash High through Agent Switchboard after explicit user authorization. If authorized, it remains a fast, cheap external workhorse for bounded search, reading, extraction, summaries, drafting, low-risk implementation/tests from an approved plan, and independent parallel packages. Use `route_agent_task` with `target_agent="antigravity"`, `surface="cli"`, `target_model="gemini flash"`, `effort="high"`, the correct `task_kind`, and `mode="plan"` or `mode="accept-edits"` plus the required implementation envelope.
...
def role_file_bodies(codex_roles: dict, claude_roles: dict) -> dict[str, str]:
    codex_luna_available = any(
        str((codex_roles.get(role) or {}).get("id") or "").strip().lower() == CODEX_CHILD_MODEL
        for role in ("frontier", "workhorse", "reader")
    )
    reader = CODEX_CHILD_MODEL if codex_luna_available else ""
    workhorse = CODEX_CHILD_MODEL if codex_luna_available else ""
    return {
