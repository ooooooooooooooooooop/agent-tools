...
- Add a broker-wide loopback Claude hook event receiver (`hook_event_server.py`): managed daemons ensure a `127.0.0.1` endpoint (default port 43827, `AGENT_BROKER_HOOK_EVENT_PORT` override) that validates `Stop`/`SubagentStop`/`StopFailure`/`SessionEnd` payloads, maps `session_id` to durable supervisor state, appends sequenced events under the shared file lock (unknown sessions go to `hook-events-orphans.jsonl` with a 202), and surfaces `hook_stop_failure`/`hook_session_end` as material events for `wait_supervisor_event`. Includes frozen-exe entrypoint wiring and receiver/integration tests.
