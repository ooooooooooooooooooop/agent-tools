...
const pressureSchema = z$1.object({
	...
	sampleValid: z$1.boolean().optional(),
	projectedInput: z$1.number().int().nonnegative().optional(),
	reservedOutput: z$1.number().int().nonnegative().optional(),
	combinedContext: z$1.number().int().nonnegative().optional(),
	configuredLimit: z$1.number().int().positive().optional(),
	...
	...sampleSource === void 0 ? {} : { sampleSource },
	...sampleStatus === void 0 ? {} : { sampleStatus },
	...estimateMethod === void 0 ? {} : { estimateMethod },
	...estimateConfidence === void 0 ? {} : { estimateConfidence }
}));
...
/** A zero/zero pre-admission/context failure sample is not a truthful occupancy observation. */
...
const usageOf = (event) => {
	const usage = event.type === "assistant/chunk" && event.data.chunk.type === "usage" ? event.data.chunk.usage : event.type === "assistant/message" && !isReplacementSurfaceEvent(event) ? event.data.usage : void 0;
...
const tokenUsageProjectionDefinition = {
	...
	apply: (state, event) => {
		...
		} else if (event.type === "assistant/message" && !isReplacementSurfaceEvent(event) && event.data.usage !== void 0) ({turn, step, usage} = event.data);
		else return state;
