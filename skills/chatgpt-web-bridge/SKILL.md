...
- **失败发送会留草稿**：`Composer text verification failed` 时插入内容可能部分留在 composer 里成为该会话的草稿，后续发送连带校验失败——清不掉就换新会话（2026-09-14 实证）
...
- 发送后拉尾部发现最后一条还是自己的 user 消息却继续轮询——回复未持久化=生成已死，立即催促续接
