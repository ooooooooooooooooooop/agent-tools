...
unified-taskflow 启动后的回读机制，防止上下文膨胀导致的目标偏移。

> 本协议只适用于已经明确启动 unified-taskflow 的重型任务。普通小任务不应为了 re-grounding 创建 `.taskflow/`。

## 触发条件（事件驱动）

以下**事件**发生时，在 checkpoint 中执行 re-grounding 核对：

| 事件 | 说明 |
|------|------|
| 关键文件创建 | 创建影响任务验收、设计、数据或行为的新文件后 |
| 关键文件修改 | 修改影响任务验收、设计、数据或行为的文件后 |
| 子任务完成 | 完成一个逻辑子任务后 |
| 用户新指令 | 用户补充新信息或修改要求后 |
| 不确定性 | 遇到模糊决策、多种可行方案时（先按 clarify-before-change 意图解析自决，不默认暂停） |
| 意图漂移 | 用户当前指令与 anchor.md Intent 语义不一致时 |

> 合并规则：连续相关文件操作应合并为一个执行批次后再 re-grounding。不要为低风险机械小改频繁打断任务。
...
2. **区分显式与模糊**：
   - 用户新指令是**显式目标变更**（用户已明确表达新方向）→ 直接更新 anchor.md Intent（Version +1）并在 checkpoint 记录，**不再回头确认**（用户显式目标优先于流程便利）
   - 新指令与 Intent 的冲突**模糊难解**（互斥价值偏好、可能误伤原目标）→ 才向用户确认：
     - A. 修改 anchor.md Intent 以匹配新方向（更新 Version）
     - B. 当前指令为临时偏离，完成后回到原 Intent
     - C. 拆分为新任务
3. **禁止**：未经判断就跟随用户隐式改变方向；也禁止对用户已明确表达的新方向做重复确认
